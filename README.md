...
[HSMA3D](https://aip.scitation.org/doi/10.1063/1.5044438) and [HSMA2D](https://aip.scitation.org/doi/10.1063/5.0003293) (with planar dielectric interfaces), which are k-space modules, have been incorporated into LAMMPS. These modules, developed in C++, are parallelized using MPI + OpenMP. To fully utilize these modules, it is recommended that users install the `user-omp` package in Lammps. Optimal performance can be achieved by utilizing fewer MPIs and more OpenMPs. It is advised not to solely rely on pure MPI. When the parameters are appropriately selected, the computational speed of this package is comparable to that of PPPM (with Intel optimization) in LAMMPS (HSMA3D), and in some cases, even faster (HSMA2D). To solve the least square problem, Intel OneAPI, or at least the Intel MKL library is required for this package. Both `make` and `cmake` (recommended) procedures are available for installation.
...
## Further discussions
Recently, we have discovered certain issues that were previously unnoticed during our interactions with users. One example is the occurrence of errors like `fortran library not found`. To address this issue, it is necessary to use commands such as `module load gcc` in order to import the latest version of gcc. This error typically arises when an updated gcc version is missing. By utilizing the `module load gcc` command, we can successfully load and utilize the most up-to-date gcc version, effectively resolving the error. 

**Moving forward, we will maintain open communication with our users and actively identify any other potential hidden issues to further enhance the quality of our codes. In addition, I am available to assist with any inquiries related to code portability in LAMMPS during my spare time. Please feel free to reach out to me with any questions or concerns you may have in this regard.**

I would like to present our recent collaborative work, which focuses on the development and implementation of the [random batch Ewald (RBE) algorithm](https://epubs.siam.org/doi/10.1137/20M1371385) and the [random batch sum-of-Gaussians (RBSOG) algorithm](https://arxiv.org/abs/2205.13824). These algorithms have been specifically designed to improve computational efficiency in the context of long-range interactions. The core concept underlying these random algorithms is straightforward yet effective. Through extensive parallel testing on CPUs with tens of thousands of cores, we have observed that both the RBE and RBSOG algorithms exhibit [a remarkable performance advantage of approximately one to two orders of magnitude](https://aip.scitation.org/doi/10.1063/5.0073424) when compared to the PPPM algorithm.

It is important to note that, at present, these algorithms are specifically tailored for three-dimensional periodic system simulations. However, we are currently involved in ongoing development efforts to broaden the applicability of these algorithms to other types of systems. For instance, we are actively working towards adapting them for use in quasi-2D systems that involve dielectric interfaces.
